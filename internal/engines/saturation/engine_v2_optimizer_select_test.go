...
var _ = Describe("selectV2Optimizer", func() {
	...
	// innerName extracts the inner optimizer name from a MultiDimensionalOptimizer,
	// falling back to opt.Name() for bare optimizers used in tests that set up
	// the Engine struct directly without wrapping.
	innerName := func(opt pipeline.ScalingOptimizer) string {
		if mdo, ok := opt.(*pipeline.MultiDimensionalOptimizer); ok {
			return mdo.Inner().Name()
		}
		return opt.Name()
	}
	...
	It("falls back to the cost-aware (unlimited) optimizer when inventory refresh fails", func() {
		...
		e := &Engine{
			optimizer:  pipeline.NewMultiDimensionalOptimizer(pipeline.NewGreedyByScoreOptimizer(), nil),
			GPULimiter: newInventoryLimiter(errors.New("failed to list nodes"), nil),
		}
		opt, constraints := e.selectV2Optimizer(ctx, nil)
		Expect(innerName(opt)).To(Equal("cost-aware"))
	...
	It("uses greedy-by-score with constraints when the inventory reports GPUs (heterogeneous pools)", func() {
		...
		opt, constraints := e.selectV2Optimizer(ctx, nil)
		Expect(innerName(opt)).To(Equal("greedy-by-score"))
	...
	It("leaves a non-greedy optimizer untouched and never queries the limiter", func() {
		...
		e := &Engine{
			optimizer:  pipeline.NewMultiDimensionalOptimizer(pipeline.NewCostAwareOptimizer(), nil),
